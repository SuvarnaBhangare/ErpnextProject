...
def get_data(
	...
	# frappe.msgprint(_(filters))
	accounts = get_accounts(company, root_type)
	# frappe.msgprint(_(accounts))
	if not accounts:
		return None
	# frappe.errprint(str(len(accounts)))
	...
	# frappe.errprint(gl_entries_by_account)
	for root in frappe.db.sql("""select lft, rgt from tabAccount
			where root_type=%s and ifnull(parent_account, '') = ''""", root_type, as_dict=1):
		# frappe.msgprint(_(root))
	...
	# frappe.errprint(str(gl_entries_by_account))
	calculate_values(
		accounts_by_name, gl_entries_by_account, period_list, accumulated_values, ignore_accumulated_values_for_fy)
	accumulate_values_into_parents(accounts, accounts_by_name, period_list, accumulated_values)
	# frappe.errprint(str(accounts))
	out = prepare_data(accounts, balance_must_be, period_list, company_currency)
	out = filter_out_zero_value_rows(out, parent_children_map)

	if out:
		add_total_row(out, root_type, balance_must_be, period_list, company_currency,filters)
...
def calculate_values(
		accounts_by_name, gl_entries_by_account, period_list, accumulated_values, ignore_accumulated_values_for_fy):
	# frappe.errprint(str(gl_entries_by_account))
	# frappe.errprint('itervalues')
	for entries in itervalues(gl_entries_by_account):
		# frappe.errprint(str(entries))
		for entry in entries:
			...
			# frappe.errprint(str(entry.project))
			for period in period_list:
				...
				if entry.posting_date <= period.to_date:
					if (accumulated_values or entry.posting_date >= period.from_date) and \
						...
						d['cost_center'] = entry.cost_center
...
def accumulate_values_into_parents(accounts, accounts_by_name, period_list, accumulated_values):
	...
	for d in reversed(accounts):
		if d.parent_account:
			# accounts_by_name['project'] = d.project
			# frappe.errprint(str(d.project))
			# frappe.errprint(str(accounts_by_name))

			for period in period_list:
				# frappe.errprint(str(period.key))
...
def add_total_row(out, root_type, balance_must_be, period_list, company_currency,filters):
	total_row = {
	...
	for row in out:
		...
		# if filters.get('project') and not row.project in list(filters.get('project')):
		# 	frappe.errprint(_("In project"))
		# 	continue
		
		# if filters.get('cost_center') and not row.cost_center in list(filters.get('cost_center')):
		# 	frappe.errprint(_("In costcenter"))
		# 	continue
...
def get_accounts(company, root_type):
	return frappe.db.sql("""select name, account_number, parent_account,lft, rgt, root_type, report_type, account_name
		from `tabAccount` 
		where company=%s and root_type=%s group by name order by lft""", (company, root_type), as_dict=True)	
...
def set_gl_entries_by_account(
	...
	gl_entries = frappe.db.sql("""select posting_date, account, debit, credit, is_opening, fiscal_year, debit_in_account_currency, credit_in_account_currency, account_currency,cost_center,project from `tabGL Entry`
		...
		as_dict=True)
	# frappe.errprint(str(gl_entries))
	if filters and filters.get('presentation_currency'):
	...
	# frappe.errprint(_(gl_entries_by_account))
