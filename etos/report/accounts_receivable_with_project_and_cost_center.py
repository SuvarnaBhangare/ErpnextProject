...
class ReceivablePayableReport(object):
	...
	def get_data(self, party_naming_by, args):
		...
		# frappe.errprint(str(gl_entries_data))
		...
		if gl_entries_data:
			...
			frappe.errprint('in')
			# frappe.errprint(str(voucher_details))
		...
		for gle in gl_entries_data:
			...
			if self.is_receivable_or_payable(gle, dr_or_cr, future_vouchers):
				...
				if abs(outstanding_amount) > 0.1/10**currency_precision:
					...
					# cost_center =  voucher_details.get(gle.voucher_no, {}).get("cost_center")
					# frappe.errprint("cost")
					# frappe.errprint(str(cost_center))
					row = [gle.cost_center]
					row += [gle.project]

					# frappe.errprint(str(cost_center)+"       "+str(self.filters.get('cost_center')))
					# if self.filters.get('cost_center') and not cost_center == self.filters.get('cost_center'):
					# 	continue
					
					# project =  voucher_details.get(gle.voucher_no, {}).get("project")
					# row += [project]
					
					# if self.filters.get('project') and not project == self.filters.get('project'):
					# 	# frappe.errprint('in_pro')
					# 	continue
					...
					# frappe.throw(str(gle.voucher_no))
	...
	def get_gl_entries(self, party_type, date=None, for_future=False):
		...
		# sql = "select name, posting_date, account, party_type, party, voucher_type, voucher_no,cost_center,project,against_voucher_type, against_voucher, account_currency, remarks, {0} from `tabGL Entry` where docstatus < 2 and party_type in {1} group by voucher_type, voucher_no, against_voucher_type, against_voucher, party order by posting_date, party".format(select_fields, tuple(values))
		...
		frappe.errprint(str(self.gl_entries))
	...
	def prepare_conditions(self, party_type):
		conditions = [""]
		values = [party_type]
		# frappe.msgprint(str(self.filters.cost_center))
		...
		if self.filters.project:
			...
			values.append(self.filters.project)
	...
	if party_type == "Customer":
		...
		for si in frappe.db.sql("""
			...
			if si.parent in dn_details:
				...
				dn_details.setdefault(si.parent, si.dn , si)
...
def get_voucher_details(party_type, voucher_nos, dn_details):
	...
	if party_type == "Customer":
		for si in frappe.db.sql("""select name, due_date, po_no
			from `tabSales Invoice`   where docstatus=1 and name in (%s)  group by name
			""" %(','.join(['%s'] *len(voucher_nos))),(tuple(voucher_nos)), as_dict=1):
				si['delivery_note'] = dn_details.get(si.name)
				voucher_details.setdefault(si.name, si)

	if party_type == "Supplier":
		for pi in frappe.db.sql("""select name, due_date, bill_no, bill_date,supplier
			from `tabPurchase Invoice` where docstatus = 1 and name in (%s) group by name
			""" %(','.join(['%s'] *len(voucher_nos))), (tuple(voucher_nos)), as_dict=1):
			voucher_details.setdefault(pi.name, pi)

	for pi in frappe.db.sql("""select name, due_date, bill_no, bill_date from
		`tabJournal Entry`  where docstatus = 1 and bill_no is not NULL and name in (%s)  group by name
		""" %(','.join(['%s'] *len(voucher_nos))), (tuple(voucher_nos)), as_dict=1):
			voucher_details.setdefault(pi.name, pi)
